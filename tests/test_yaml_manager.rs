...
"#;
    let context = default_context();
    let result: cdumay_error::Result<TestYamlConfig> = YamlManager::read_str(yaml, &context);
...
fn test_yaml_manager_read_failure() {
    ...
    let result: cdumay_error::Result<TestYamlConfig> = manager.read(reader, &context);
    assert!(result.is_err());
}
